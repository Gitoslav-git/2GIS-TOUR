...
Версия `0.5` — проверяемый этап разработки полного MVP `1.0`. Главный источник требований — `docs/source/2ГИС Системная аналитика.docx`; решения и открытые вопросы записаны в `docs/DECISIONS.md`, границы выпусков — в `docs/RELEASES.md`. Для прибытия к точке принято N = 75 м; само прохождение начинается в 0.6.
...
Кнопка **«Определить геопозицию»** запрашивает системное разрешение только по действию пользователя. В API-06 уходят координаты и точность измерения; без разрешения используется явно обозначенный приблизительный старт. Запрос, текущий маршрут и геопозиция сохраняются при повороте главного экрана. Карта хранит ответ backend в `ViewModel`, а `MapView` восстанавливает камеру, поэтому поворот не создаёт лишний запрос и не сбрасывает маршрут.
...
1. Запустите backend и задайте геопозицию эмулятора в Туле или Владимире.
2. Нажмите **«Определить геопозицию»**: приложение должно показать координаты и точность.
...
5. Сбросьте либо запретите геопозицию: маршрут должен строиться от выбранной области города с явным предупреждением.

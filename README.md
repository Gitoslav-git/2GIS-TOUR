...
## Что проверять в 0.5.4

1. Проверьте четыре варианта старта из `docs/FIXES-0.5.4.md`; источник старта должен быть явно написан в результате.
2. В Москве запрос «от МЦК Кутузовская в сторону центра 2 часа» должен начинаться от найденной станции, даже если геопозиция доступна.
3. Поменяйте порядок двух точек и примените: редактор должен закрыться, а переходы и время — пересчитаться.
4. Нажмите **«Сбросить маршрут»**, подтвердите удаление и убедитесь, что запрос и кнопки текущего маршрута очищены.
5. Откройте карту и поверните экран: камера, введённый запрос и текущий маршрут не должны исчезнуть.
6. Соберите APK без мобильного ключа: кнопка карты должна показать понятное сообщение о `dgissdk.key`, а не завершить приложение.
